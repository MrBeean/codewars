def meeting(s)
  array = s.split(';').map { |names| names.split(':') }.map { |names| names.map { |name| name.upcase } }
  array.map! do |names|
...
meeting(string) # =>"(CORWILL, ALFRED)(CORWILL, FRED)(CORWILL, RAPHAEL)(CORWILL, WILFRED)(TORNBULL, BARNEY)(TORNBULL, BETTY)(TORNBULL, BJON)"
